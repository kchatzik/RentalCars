...
public class VehicleHandler {
    public static ArrayList<Vehicle> getVehiclesByType(ArrayList<Vehicle> vehicleArrayList, char type, VehicleSpecifications specs){
        ...
        for(Vehicle vehicle : vehicleArrayList) {
            ...
            String vehicleTypeString = specs.getVehicleTypeByLetter(sippArray[0]);
            char vehicleTypeChar = vehicleTypeString.charAt(0);

            if (vehicleTypeChar == type) vehiclesWithTheType.add(vehicle);
    ...
    public static ArrayList<Character> getVehicleTypes(ArrayList<Vehicle> vehicleArrayList, VehicleSpecifications specs){
        ...
        for(Vehicle vehicle : vehicleArrayList){
            ...
            if(!vehicleTypesArrayList.contains(Character.valueOf(vehicleTypeChar))){

                vehicleTypesArrayList.add(Character.valueOf(vehicleTypeChar));
